...
struct matmul_template {
    ...
    __device__ static inline void common_setup(common_setup_args<layout> args) {
        ...
        // extract the block tile id from task_id
        if (task_id < super_rows * Cblocks) // 32*16 = 512
            // the original requires some trick, I explicitly write out what happened during index extraction
            // the original one was:
            // SUPER_M*(task_id/super_repeat) + task_id%SUPER_M
            // (task_id % super_repeat) / SUPER_M
            args.common.coord = { SUPER_M*(task_id/super_repeat) + (task_id - (task_id/super_repeat) * super_repeat)%SUPER_M, (task_id - (task_id/super_repeat) * super_repeat)/SUPER_M };
        else if (task_id < Rblocks*Cblocks) { // 512
            int remainder_id = task_id - super_rows*Cblocks;
            args.common.coord = { super_rows + (remainder_id%final_rows), remainder_id/final_rows };
        }
        else { // Id is too high, no more work to do
            // This is another confusing point. The reason for this check is due to persistent block pattern. Where we initialize 132 logical block container
            // And within wach block container, we iteratively load actual blocks onto it per wave
            // If I have 256 actual blocks and 132 block container
            // some of the block container will not read any blocks at the last wave and the iteration will return
            // However, some of the block container successfully read one actual block and thus will let the loop keep going
            // then, in the last iteration, every task id will be larger than total block, and all block containers iteration will exit
            args.num_iters = -1;
            return;
        }
        args.num_iters = args.globals.A.cols()/64;  // 64
        // Here, the logic is, we have NUM_CONSUMER_WARPS consumers, divide by 4 to get consumer warpgroups
        // So, consumer warpgroup will be indexed from [0, NUM_CONSUMER_WARPS/4). And we usually will create
        // another warpgroup for producer. So, when warpgroup::groupid() == NUM_CONSUMER_WARPS/4 is true, we found
        // the producer, And the producer will be responsible for loading data from a and b
        // other consumers still get their own id, for getting the subtask each of them will compute
        int id = warpgroup::groupid() == NUM_CONSUMER_WARPS/4 ? 0 : warpgroup::groupid(); // producer sets as 0
        // tk is finally on warp level, so, this coord will be transformed to warp index consisted of the id obtained above
    ...
    struct producer {
        ...
        __device__ static void load(producer_load_args<layout> args) {
            if(warpgroup::warpid() == 0) {
                ...
                // load the 128 by 64 tiles from A
                for(int i = 0; i < M_BLOCK; i++)
                    tma::load_async(args.input.a[i], args.globals.A,
                                    {args.common.coord.x+i, args.iter}, args.inputs_arrived);
                // load the 64 by 256 tiles from B
    ...
    struct consumer {
        ...
        __device__ static void compute(consumer_compute_args<layout> args) {
            ...
            // the actual implementation is at ThunderKittens/include/ops/group/wgmma/wgmma.cuh, which has a loop
            warpgroup::mma_ABt(
